...
keywords: ["copy a process", "clone a process", "crash loop", "stop a process"]
---
...
| **duplicate** | Copy the process on the same machine. Every setting is carried over except the name, which gains a ` (copy)` suffix, and the launch mode, which is forced to `off` so the copy cannot start a second instance of the same executable behind your back. | `MACHINE_CONFIG_WRITE` |
...
A process in `scheduled` launch mode runs inside **blocks**: a set of days plus one or more start/stop ranges. A range whose stop time is earlier than its start is an overnight window and carries into the next day.

The editor previews the whole week as a **seven-day bar** — one column per day, with a coloured segment for every range. Each block keeps its own colour, so a process with a morning block and an evening block reads at a glance, and a pattern that accidentally covers only four days is obvious before you save. The same bar appears on the process row and on the schedule preset list.

Rather than rebuild a common pattern on every machine, save it once as a [schedule preset](/docs/dashboard/admin/schedule-presets) and apply it. Four presets are built in, including `business hours` and `24/7`.

Windows are wall-clock rules, not absolute instants. Which clock they are read against depends on the site — see [timezones](/docs/dashboard/timezones#process-schedules).
