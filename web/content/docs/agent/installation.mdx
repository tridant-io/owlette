...
9. On the final wizard page, leave **open owlette** checked and click Finish to bring the window back up. That is not a second pairing step, and silent installs skip it entirely.
...
| `/NOSERVICECONTROL` | Keep the administrator prompt for starting and stopping the service. See [administrator prompts](/docs/agent/system-tray#administrator-prompts). |

<Callout type="warning" title={"/NOSERVICECONTROL is sticky"}>

It writes `HKLM\SOFTWARE\Owlette\AllowLocalServiceControl = 0`, which later agent
updates never clear — re-enabling local service control on a machine somebody
deliberately locked down is not something a background update should decide. To
undo it, delete that value and restart the service.
...
flowchart TD
    ...
    B -->|"/ADD= phrase, or interactive<br/>without WebView2"| C["runs configure_site.py<br/>on the console"]
    ...
    C --> S
    Z --> S
    D --> E["phrase, authorization link, environment,<br/>and 10-minute expiry are shown;<br/>polling starts immediately"]
    C --> E
