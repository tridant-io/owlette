---
title: "installation"
description: "there are four supported ways to install or update the owlette agent. one installer version ships one file per platform: Windows, macOS on Apple silicon, and Linux."
keywords: ["add a machine", "new machine", "onboard", "macos", "linux"]
---

1. Interactive packaged installer for a single machine
2. Silent packaged installer with `/ADD=` for bulk enrollment (Windows only)
...
2. Double-click the package, or install it from a terminal:

```bash
sudo installer -pkg Owlette-Installer-v<version>.pkg -target /
```

3. Grant **Screen Recording** to the owlette app when macOS asks; the app shows a notice until it is granted.
4. Pair from the owlette app after installing — the phrase and its link are shown there, and you authorize as in the Windows steps above.
...
The service runs as `owlette-agent.service`, and the app starts for every login through a user unit. Pair from the owlette app after installing.

---

## method 2: silent install with `/ADD=`

Use this path for bulk deployment when an admin has already generated and authorized a pairing phrase. The silent install is Windows only; on macOS and Linux, pair from the app after installing.
...
| Permissions | Administrator privileges (Windows) or `sudo` (macOS, Linux) for service installation |
