...
2. A **reboot countdown prompt** appears on screen (the desktop app, launched with `--restart-prompt`), and the agent writes a `rebootPending` flag that raises the [restart pending banner](/docs/dashboard/machine-monitoring#restart-pending) on the dashboard, where an admin can approve or dismiss it remotely
...
| **GPU** | NVML (`nvidia-ml-py`) | GPU usage and VRAM percentage (if available) |
...
- **GPU usage/VRAM**: pynvml/NVML (NVIDIA only)
