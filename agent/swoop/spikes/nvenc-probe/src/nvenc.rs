...
use moq_nvenc::sys::nvEncodeAPI::{
    ...
    NV_ENC_BUFFER_USAGE, NV_ENC_CAPS, NV_ENC_CAPS_PARAM, NV_ENC_CAPS_PARAM_VER, NV_ENC_CODEC_CONFIG,
...
/// How many NVENC "two pass" settings the spike compares. The product default
/// in plan.md D7 / Task 3.7 is quarter-resolution two-pass; the spike measures
/// whether that costs latency on this part.
...
impl MultiPass {
    fn to_sys(self) -> NV_ENC_MULTI_PASS {
        match self {
            MultiPass::Disabled => NV_ENC_MULTI_PASS::NV_ENC_MULTI_PASS_DISABLED,
            MultiPass::QuarterResolution => NV_ENC_MULTI_PASS::NV_ENC_TWO_PASS_QUARTER_RESOLUTION,
        }
    }
}
...
pub struct EncoderConfig {
    ...
    /// Async encode with depth 1: submit one picture, wait on a Win32
    /// completion event, lock. `false` runs the same loop synchronously, where
    /// `nvEncLockBitstream` itself blocks — the control that says whether the
    /// event round trip costs anything.
    pub async_encode: bool,
    /// H.264 `level_idc`, or 0 to let NVENC pick. It is here because without
    /// the VUI fix the decoder's output delay is the level's implied maximum
    /// DPB, so the level is what sets the size of the penalty.
    pub h264_level: u32,
}
...
pub struct Encoded {
    ...
    /// event in async mode, or to `nvEncLockBitstream` returning in sync mode.
    pub encode: Duration,
...
impl Session {
    ...
    pub fn open(device: &ID3D11Device, cfg: EncoderConfig) -> Result<Self> {
        ...
        let mut init = NV_ENC_INITIALIZE_PARAMS {
            ...
            enableEncodeAsync: u32::from(cfg.async_encode),
    ...
    /// Query one encoder capability, for the `caps` report.
    pub fn caps(&self, which: NV_ENC_CAPS) -> Result<i32> {
        let codec_guid = match self.cfg.codec {
            Codec::H264 => NV_ENC_CODEC_H264_GUID,
            Codec::Hevc => NV_ENC_CODEC_HEVC_GUID,
        };
        ...
        let mut value: i32 = 0;
        check(
            unsafe { (ENCODE_API.get_encode_caps)(self.encoder, codec_guid, &mut param, &mut value) },
            "nvEncGetEncodeCaps",
        )?;
        Ok(value)
    }
...
fn apply_rate_control(config: &mut NV_ENC_CONFIG, cfg: &EncoderConfig) {
    ...
    rc.multiPass = cfg.multi_pass.to_sys();
