...
//   - .tokens.enc -> SYSTEM + Administrators, plus the interactive console user
...
begin
  ...
  PS :=
    ...
    '  } else { Wr("skip (absent): $p"); Wr("") }' + #13#10 +
    '}' + #13#10 +
    '# credentials: add the console user (Modify) only when a session exists.' + #13#10 +
    '$tok = Join-Path $DataRoot ".tokens.enc"' + #13#10 +
