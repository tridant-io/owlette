...
class TestSessionChangeRepair:
    ...
    def console(self, monkeypatch):
        ...
        session = {'id': 1}
        monkeypatch.setattr(
            owlette_service.win32ts, 'WTSGetActiveConsoleSessionId', lambda: session['id'])
        monkeypatch.setattr(
            owlette_service.win32ts, 'WTSQueryUserToken', lambda session_id: MagicMock())
        monkeypatch.setattr(
            owlette_service.win32profile, 'CreateEnvironmentBlock', lambda token, inherit: {})
        return session
...
class TestConsoleWatch:
    ...
    def test_the_console_read_is_the_session_and_its_user(self, monkeypatch):
        import owlette_service
        monkeypatch.setattr(owlette_service.win32ts, 'WTSGetActiveConsoleSessionId', lambda: 3)
        monkeypatch.setattr(owlette_service.win32ts, 'WTSQuerySessionInformation',
    ...
    def test_a_detached_or_unreadable_console_reads_as_none(self, monkeypatch):
        import owlette_service
        monkeypatch.setattr(
            owlette_service.win32ts, 'WTSGetActiveConsoleSessionId', lambda: 0xFFFFFFFF)
        assert owlette_service._console_session() is None

        monkeypatch.setattr(owlette_service.win32ts, 'WTSGetActiveConsoleSessionId', lambda: 1)

        def fail(*args):
            raise OSError('rpc unavailable')

        monkeypatch.setattr(owlette_service.win32ts, 'WTSQuerySessionInformation', fail)
        assert owlette_service._console_session() is None
