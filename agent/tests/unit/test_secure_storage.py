...
file under pytest's ``tmp_path`` holding dummy values only; the machine binding
behind ``osadapter`` is fixed. SID conversions use the real pywin32, a pure
operation.
...
# stands in for MachineGuid, so the derivation under test is the test's to control.
_BINDING = b'00000000-0000-0000-0000-000000000000'
...
def storage(tmp_path, monkeypatch):
    monkeypatch.setattr(osadapter, '_adapter', SimpleNamespace(
        key_material=lambda: _BINDING,
        stable_machine_id=lambda: _BINDING.decode(),
    ))
    return secure_storage.SecureStorage(config_dir=tmp_path)


def _pre_migration_blob(payload):
    """A token store as the hostname-mixed derivation wrote it, spelled out
    rather than taken from secure_storage."""
    material = _BINDING + b':' + platform.node().encode('utf-8') + b':owlette-agent'
    cipher = Fernet(base64.urlsafe_b64encode(hashlib.sha256(material).digest()))
...
class TestFailClosed:
    ...
    def test_a_missing_spec_table_writes_nothing(self, storage, tmp_path, caplog):
        ...
        with _writer(writer=None, console=None) as recorder, \
             patch.object(acl_hardening, 'specs', return_value=[]):
            assert storage.save_refresh_token(_SECRET) is False
...
class TestPreMigrationCopy:
    """The copy kept across the key-derivation change is a credential store
    too: it is written through the protected path, under the store's DACL, and
    the start-up repair table carries it with the store's ACEs."""

    def test_the_copy_and_the_store_are_written_alike(self, storage):
        ...
        retained = storage._retained_file()
        assert retained.read_bytes() == original
        assert storage.token_file.read_bytes() != original
        assert len(recorder.calls) == 2, 'the copy and the store, both through the writer'
        copy, store = recorder.calls
        temp = r'\.[0-9a-f]{8}\.tmp$'
        assert re.match(re.escape(str(retained)) + temp, copy.path)
        assert re.match(re.escape(str(storage.token_file)) + temp, store.path)
        assert copy.dacl_first is True and store.dacl_first is True
        assert copy.aces == store.aces == [
            (_SYSTEM, _FULL, 0),
            (_ADMINISTRATORS, _FULL, 0),
            (_CONSOLE, _MODIFY, 0),
        ]
    ...
    def test_the_repair_table_carries_the_copy_with_the_stores_aces(self):
        ...
        copy = rows[secure_storage.TOKEN_FILE_NAME + secure_storage.PRE_MIGRATION_SUFFIX]
        assert copy.aces == store.aces
        assert copy.app_root is store.app_root is False
        assert os.path.dirname(copy.path) == os.path.dirname(store.path)
