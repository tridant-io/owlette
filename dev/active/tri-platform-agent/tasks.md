...
**Progress (2026-09-25, reconciled against the repo)**: 14/33 boxes ticked (was 9/33; 0.7 stays `[~]`). Ticked this pass: 3.1, 3.2, 3.4, 3.7 (PR #167 → dev, VM/MBA-proven) and 4.1 (PRs #211/#212). Every other unchecked task carries a `— open:` line naming exactly what is left; see the 2026-09-25 reconciliation Log entry.

**Progress (2026-09-25, after the overnight run)**: Wave 0 — 0.2 (a)/(b) measured on the MBA, 0.4 Linux half met on the kiosk VM (reboot), macOS half short of a reboot; Wave 4 — 4.1/4.2/4.6/4.7 posix arms merged (#211/#212), 4.3 job runner and the macOS half of 4.4 merged (#231), macOS title bar / menubar / Dock behaviour (#228/#229), login-item deferral (#233); Wave 5 — 5.1 unsigned `.pkg` built and inspected (#232), 5.2 `.deb`s built, installed and verified on the kiosk VM (#235); Wave 6 — the machines API carries `osFamily`/`arch`/`osVersion` (#237). Afternoon: the MBA paired (`TEC-MBA.local`), Screen Recording granted, team id `69QC59ZB54` given, 5.1 signed + notarized + stapled (#242), one 0.6 timing sample. Owed to the owner: the sudo install of the `.pkg`, a Mac reboot, the `.local` decision.
...
- [ ] **Task 0.2: macOS capture + TCC spike** (gates waves 3, 4, 5, 8)  **[verbatim: H §15]**
  - — open: grant survival across a reboot, (c) the LaunchDaemon-spawned `mss`, the 30-day re-authorisation (Q15) and the line counts — all need the owner at the laptop (`spikes/macos-tcc.md` "still owed").
...
- [ ] **Task 0.3: Linux capture spike** [title inferred]
  - — open: no `spikes/linux-capture.md` was ever committed (git history holds only `macos-tcc.md`); Linux capture is not built — `desktop/src-tauri/src/jobrunner.rs` answers `unsupported_job` on linux (Q5).
...
- [ ] **Task 0.4: user-unit spike** (gates decision 2, wave 4)  **[verbatim: H §15]**
  - — open: the macOS half — reboot / auto-login survival and a notification from a signed bundle (needs the 5.1 install and a Mac reboot). Linux half met on the kiosk 2026-09-25.
  ...
  - Depends on: nothing.
  - Status 2026-09-25 (Linux half, kiosk VM): **MET.** After `sudo systemctl reboot` the service was active at boot and the globally-enabled `owlette-desktop` user unit came up 16 s later on GDM autologin (Xorg `:0`, `XAUTHORITY=/run/user/1000/gdm/Xauthority`), pid 2025 = `tmp/tray.pid`; a `notify` job through the daemon delivered. The root-spawned GUI child was measured by the lab earlier (T#166).
...
- [ ] **Task 0.5: pytest matrix spike** [title inferred]
  - — open: could not settle — no `spikes/pytest-matrix.md` was ever committed; the three-OS matrix it gated shipped in 2.4 (`.github/workflows/agent-tests.yml`).
...
- [ ] **Task 0.6: notarization timing spike** (gates wave 5)  **[verbatim: H §15]**
  - — open: the p50/p95 five-run measurement from a GitHub macOS runner (one MBA sample: about 2 min submit→staple). Done already: the eight `APPLE_*` names in `scripts/env-manifest.json` `github-repo-secrets` and the job graph in `.github/workflows/build-installer.yml` (commit 21f2e50c, this branch).
...
- [~] **Task 0.7: fleet-floor spike** [title inferred]
  - — open: the prod fleet floor (the prod key is installer-scoped); the dev half is recorded.
...
- [x] **Task 3.1: `osadapter/posix.py`**  **[verbatim: H §15]**
  - — shipped: fc330739 / 21ad3fea / dd7b5106 → PR #167 (47e5cae0) → dev; darwin arms 4b7618e5; `agent/src/osadapter/posix.py` on origin/dev; the contract test runs on all three legs of `.github/workflows/agent-tests.yml`; kiosk VM proofs in the 2026-09-16 Log. The macOS root-daemon checks (H §16.4) wait on the 5.1 install.
...
- [x] **Task 3.2: `osadapter/darwin.py` and `osadapter/linux.py`**  **[verbatim: H §15]**
  - — shipped: `linux.py` fc330739 (PR #167); `darwin.py` 4b7618e5 … c2415865 (on origin/dev); `streamer_capable()` arms in all three adapters; agent suite 1766/225 on the MBA; the kiosk runs the packaged runtime (PR #235).
...
- [x] **Task 3.4: service wiring on POSIX**  **[verbatim: H §15]**
  - — shipped: fc330739 (PR #167); `installer_utils.verify_artifact_family` (`:473`, `xar!` / `!<arch>`), the SIGBREAK guard `owlette_runner.py:228`, the macos/ubuntu `--ignore` lists in `agent-tests.yml` down to the display + swoop entries; the deb self-update path proven on the kiosk (multi-platform-releases 1.2, 2026-09-25). The pkg path is unexercised until the 5.1 install.
...
- [x] **Task 3.7: pairing on POSIX (`configure_site.py`)**  **[verbatim: H §15]**
  - — shipped: fc330739 / 21ad3fea (PR #167); `configure_site.py --preseed`, `config/pairing.json` / `OWLETTE_ADD`, consumed by `debian/postinst` and `macos/scripts/postinstall`; preseed pairing on the kiosk 2026-09-17 and again from the .deb (PR #235, `owlette-kiosk` online); the MBA paired 2026-09-25 (`TEC-MBA.local`, from source — the pkg preseed waits on the 5.1 install).
...
- [x] **Task 4.1: cargo gating, paths, shell_open**  **[verbatim: H §15]**
  - — shipped: PR #211 (e1faff41) + PR #212 (10703c87): `Cargo.toml` `cfg(windows)` / `cfg(unix)` targets, `tauri.conf.json` `targets` / `signingIdentity: null` / `minimumSystemVersion: "15.0"`, `paths.rs` `data_root()` / `install_root()`, `shell_open.rs` `open` / `xdg-open`; the three `cargo check` legs are `rust-build.yml` (`crates` on windows, `desktop-posix` on ubuntu + macos).
...
- [ ] **Task 4.2: service_ctl, process_ctl, startup_link**  **[verbatim: H §15]**
  - — open: restart through the `ipc/requests` seam (deviation 3) — `tray.rs:1194` and `App.tsx:487` still write `tmp/restart.flag`, a no-op off windows; the macOS relaunch-at-login proof (needs the 5.1 install). Shipped so far: PR #212 (10703c87) posix arms + CI, PR #233 (71b83fef) login-item deferral, the polkit rule proven on the kiosk.
...
- [ ] **Task 4.3: the POSIX job runner**  **[verbatim: H §15]**
  - — open: the `shell` / `launch` job bodies (`jobrunner.rs:256` refuses them; the daemon sends only `capture` and `notify`, `posix.py:280,302`) and Linux capture (`unsupported_job`, Q5). Shipped so far: PR #231 (78c50ef2) `jobrunner.rs` capture/notify + `json_io.rs` flock, measured on the MBA.
  ...
  - Depends on: 4.1.
  - Status 2026-09-25: **REWRITTEN AND MEASURED on the MBA** (the `w4a` copy was lost with its worktree). `jobrunner.rs` (unix, ~330 lines, polled at 100 ms rather than watched): accepts a regular ≤64 KiB request writable by nobody but its owner, `capture` (macOS `screencapture -D` on the app's grant; Linux `unsupported_job` until Q5) and `notify`, `trusted` honoured only for a root-owned request, every other type a typed refusal, result written whole then renamed (dir 0750, file 0640). Hand-written jobs on the Mac: capture answered in 180 ms (typed `capture_failed` without the grant), notify 10 ms, untrusted shell refused in 6 ms. 7 unit tests; 118/118 on macOS, clippy clean. `shell`/`launch` bodies are not written (nothing in the daemon sends them yet).
  - Status (reconstructed, before that): **DONE-ON-DISK-ONLY, fix/verification IN PROGRESS at 23:12** in `w4a` [inferred: D, I].
...
- [ ] **Task 4.4: capture provider and TCC surface**  **[verbatim: H §15]**
  - — open: `capabilities.screenCapture` is not in the heartbeat (`firebase_client.py:1573-1576` writes only `displayRemoteApply` and `swoop`), so the dashboard never sees the grant; the Linux wayland/x11 banner reason. Shipped so far: `tcc.rs` (PR #231), `PermissionBanner.tsx` (PR #239, 1d6ef69e), the darwin `ipc/tcc.json` reader and the `streamer_capable()` arms.
  ...
  - Status 2026-09-25: **tcc.rs DONE (macOS)** — asks once per launch (`CGRequestScreenCaptureAccess`, which is what lists the app in Settings; `screencapture` never asks), writes `ipc/tcc.json` `{screen_recording, checked_at}` every 60 s, 0644, whole-then-rename; measured on the MBA (`false` until the owner grants owlette.app). The capture itself lives in `jobrunner.rs` (4.3), ~40 lines on `screencapture -D`, no new crate — spike 0.2's go. `PermissionBanner.tsx` not started.
  - Status (reconstructed, before that): **NOT STARTED**. No `capture.rs`, `tcc.rs` or `PermissionBanner.tsx` exists in either lane [D]. The daemon-side TCC contract is fixed by H §16.2 (`ipc/tcc.json`), and H §16.3 adds research amendments.

- [ ] **Task 4.5: desktop frontend per-OS handling**  **[verbatim: H §15]**
  - — open: per-OS `DEFAULT_CLASSIFY_OPTIONS` (`dropClassifier.ts:99` is one object) and a case-sensitive `pathKey()` off windows (`dropQueue.ts:148` still lowercases). Shipped so far: native decorations on macOS (PR #228, `tauri.macos.conf.json`).
...
- [ ] **Task 4.6: Rust CI on three OSes**  **[verbatim: H §15]**
  - — open: the ubuntu arm64 leg only (`linux_arm64` reserved — multi-platform-releases decision 1: no runner, no fleet). Shipped: PR #212 (10703c87) `rust-build.yml` `desktop-posix` matrix ubuntu/macos beside the windows `crates` job, `working-directory:` per crate; three red CI rounds proved the gate.
...
- [ ] **Task 4.7: agent CLI bridge and update marker on POSIX**  **[verbatim: H §15]**
  - — open: pair / restart through the `ipc/` requests (the join dialog still runs `configure_site.py` directly; `App.tsx:487` writes `restart.flag`). Shipped so far: `agent_cli.rs` resolves the interpreter per OS from the install root (PR #212, `:47-53`); `serviceHealth.ts:281` reads the marker data-root-relative.
...
- [ ] **Task 5.0**: [unknown]. A dependency of 5.1 [verbatim: H §15]. Status: **UNKNOWN / not started** [inferred].
  - — open: could not settle — the task text was never recovered; nothing to verify against.

- [ ] **Task 5.1: macOS `.pkg`**  **[verbatim: H §15]**
  - — open: the pkg install itself (sudo on the MBA), the clean-VM Done-when, `uninstall.sh`, `pmset`, the `com.apple.servicemanagement` profile (Q16). Shipped: PR #232 (7fb101f6) `build.sh` + plists + `distribution.xml` (arm64, 15.0) + pre/postinstall with the preseed; PR #242 (ad99cc81) inside-out signing, notarized + stapled; a180b6c4 (this branch) `--notary-key*` flags + the PBS sha pin.
  ...
  - Status 2026-09-25: **UNSIGNED PATH BUILT AND INSPECTED on the MBA.** `agent/build/macos/build.sh` (python-build-standalone `3.11.16+20260924` arm64, pip from `agent/requirements.txt` with its posix markers, `agent/src` staged, two component pkgs + one product via `pkgbuild`/`productbuild`, `--installer-identity` / `--notarize` flags), `agent/packaging/macos/{app.owlette.agent.plist, app.owlette.desktop.plist, distribution.xml, welcome.txt, scripts/preinstall, scripts/postinstall}`. First product: `Owlette-Installer-v3.3.15.pkg`, 132 MB, 5,025 files, plists at `/Library/LaunchDaemons` and `/Library/LaunchAgents`, runtime under the data root, app to `/Applications`, arm64 + 15.0 gate, scripts parse, no win32 package leaked. **Not done:** an install (needs root on the laptop — no admin password over ssh), the Done-when on clean VMs, signing/notarization (Q4), the `com.apple.servicemanagement` profile (Q16). Open question: the app's own "start on login" toggle writes `~/Library/LaunchAgents/app.owlette.desktop.plist`, the same label as the system LaunchAgent — on macOS the toggle should become a no-op or the labels must differ.
  - Status 2026-09-25 (afternoon): **SIGNED AND NOTARIZED on the MBA** (PR #242, branch `tri-platform/macos-signing`). First submission with only the app signed came back Invalid, 93 issues, every one a Mach-O inside `runtime/python` (no Developer ID, no hardened runtime, no secure timestamp, 2 unsigned). `build.sh` now signs the runtime inside-out when `APPLE_SIGNING_IDENTITY` is set (dylibs/.so, then executables, `--options runtime --timestamp`, the interpreter with `agent/packaging/macos/entitlements.plist`: allow-jit, allow-unsigned-executable-memory, disable-library-validation). Second submission `d6981a1f-73be-494d-851c-e1c06f3a153b` → Accepted, stapled; `spctl -a -t install` → `accepted, source=Notarized Developer ID`; `pkgutil --check-signature` → trusted timestamp 2026-09-25 19:59:51 UTC, chain Developer ID Installer: Dylan Roscover (69QC59ZB54). Product sha256 `c2c882ba7bc1dbb846e06930706bc9d5074078b33c5ed7b9459d3a37a680eece`, copied to `~/Desktop` on the MBA for the owner's sudo install. Signing over ssh is impossible (locked keychain, `errSecInternalComponent` / `keychainLocked`): the build runs as a LaunchAgent job in the owner's GUI domain (`app.owlette.build`, log `~/src/signed-build.log`); identities in `~/Library/Keychains/moonshine-signing.keychain-db`, notarytool profile `owlette-notary` (app-specific password). **0.6 data point:** 132 MB product, submit→Accepted→staple in about 2 minutes on the MBA (signature 19:59:51, stapled pkg mtime 20:01 UTC); one sample, the GitHub-runner five-run measurement is still owed. **Still not done:** the install itself (sudo), the Done-when on clean VMs, Q16.
  - Also measured 2026-09-25: the daemon's own path through the real app on the Mac (`posix.capture_screen` / `posix.notify` from `agent/src` with `OWLETTE_DATA_ROOT=~/owlette-data`, the app from the desktop-spike LaunchAgent): capture answered in 345 ms with the typed no-grant refusal, notify delivered in 105 ms, request and result cleaned up by the daemon. The Hyper-V kiosk VM (`owlette-kiosk`, Ubuntu 24.04 x86_64, 172.22.80.231, key `C:\VMs\owlette-kiosk\ssh\id_ed25519`, user `kiosk`, passwordless sudo) still runs the lab agent 3.3.5 from `/opt/owlette/venv`; it has no Rust or Node, so a Linux app or `.deb` must be built elsewhere and copied in.
  - Status (reconstructed, before that): **NOT STARTED** (blocked on Q4 for signing). Amended from the Mac [verbatim: H §16.3]: arm64 only with the floor at 15.0 (Done-when VMs are Sequoia + Tahoe, refusal observed on Sonoma); `AssociatedBundleIdentifiers` in both plists; sign the bundled interpreter with the Team ID; no `/bin/sh` wrapper; strip xattrs (macOS 27 refuses quarantined plists); `syspolicy_check` / `gktool scan`; `disable-library-validation` only if something is pip-installed on the device; `NSLocalNetworkUsageDescription`. The macOS self-update is now a run-once plist job (`57802bce`), not `launchctl submit`.

- [ ] **Task 5.2: Linux `.deb`** [title inferred]
  - — open: arm64 / Raspberry Pi OS Bookworm (`linux_arm64` reserved — multi-platform-releases decision 1) and `systemctl mask sleep.target` (not in `postinst`). Shipped: PR #235 (3a955789) units (`KillMode=process`, `WantedBy=graphical.target`, `ConditionGroup=owlette`), `49-owlette.rules`, postinst mode table + `json.lock` + preseed; a180b6c4 (this branch) one merged `Owlette-Installer-v<ver>.deb` with `Provides/Replaces/Conflicts: owlette`; kiosk: install, reboot survival, polkit, pairing, self-update from one deb.
  ...
  - Done when / Depends on: [unknown]
  - Status 2026-09-25: **BUILT, INSTALLED AND VERIFIED on the kiosk VM** (`owlette-kiosk`, Ubuntu 24.04 x86_64, toolchain installed there that night: rustup 1.98.1, Node 22 under `~/.local/node22`, the Tauri apt deps). `agent/build/linux/build.sh` → `owlette-agent_3.3.15_amd64.deb` (128 MB; python-build-standalone 3.11.16, `/opt/owlette/{python,agent}`, `/usr/lib/systemd/system/owlette-agent.service` with `KillMode=process` + `WantedBy=graphical.target`, `/usr/lib/systemd/user/owlette-desktop.service` with `ConditionGroup=owlette` and no `Restart=always`, `/etc/polkit-1/rules.d/49-owlette.rules` scoped to the unit and the group, `Depends: owlette (>= ver)`) beside the Tauri `owlette_3.3.15_amd64.deb` (5.7 MB). `apt install` of both over the lab 3.3.5 (its unit and `/opt/owlette` moved aside): service active from the new runtime, `STARTUP COMPLETE … Version 3.3.15 … Firebase connected`, dev API shows `owlette-kiosk` online on 3.3.15; user unit enabled+active for the seat, `tmp/tray.pid` = the app's pid; `posix.notify` through the daemon `{'status': 'sent'}`; `systemctl restart owlette-agent` as the seat user succeeded (polkit); the mode table held. **Not done:** arm64, a reboot (auto-login survival), `apt-get install --simulate` self-update path (3.4), `linux.deb.depends` in `tauri.conf.json` (the Tauri deb declared its own runtime deps and installed cleanly). Note: the service log says "Running under owlette-host" on POSIX — the detection is wrong off Windows (cosmetic; the SCM watcher is Windows-only anyway).
  - Status (reconstructed, before that): **NOT STARTED**. The `.deb` bundle config exists only in 4.1's on-disk `tauri.conf.json` (`linux.deb.depends`) [D].

- [ ] **Task 5.3: release workflow restructure**  **[verbatim: H §15]**
  - — open: the tag run — three artifacts under one provenance and `slsa-verifier` on each (blocked on the eight Apple secrets + the `release` environment). Shipped: `build-installer.yml` `build-windows` / `build-macos` / `build-linux` → `digest` → `provenance` → `release` → `verify`, commit 21f2e50c on this branch (multi-platform-releases 4.2), zizmor clean.
...
- [ ] **Task 5.4, 5.5**: [unknown]
  - — open: could not settle — the task text was never recovered.
- [ ] **Task 5.6: dev deploy hook on the new layout** [title inferred]
  - — open: not started — `.claude/hooks/deploy-agent.mjs:37` still copies by basename, so `osadapter/` is not mirrored; needs an explicit request (CLAUDE.md).
...
- [ ] **Task 6.1: heartbeat `osFamily` / `osVersion` / `arch` + `capabilities.screenCapture|packageInstall|temps`** [title inferred]
  - — open: the `capabilities.screenCapture|packageInstall|temps` keys — the heartbeat writes only `capabilities.displayRemoteApply` and `capabilities.swoop` (`firebase_client.py:1573-1576`). Shipped: f56487b8 (`osFamily` / `arch` / `osVersion` at registration + heartbeat, machine card and list); PR #237 (a809db99) exposes them on the machines API.
...
- [ ] **Task 6.4: installation docs** [title inferred]
  - — open: the preseed (`pairing.json` / `OWLETTE_ADD`) appears in no docs page; Q24 (a display-manager session is required, startx is not a seat); the login-items toggle without MDM; the MDM section (Q-M3). Shipped: `web/content/docs/agent/installation.mdx` per-platform windows/macos/linux sections + `/download?os=` (commit 21f2e50c, multi-platform-releases 4.3).
...
### 2026-09-25 — reconciliation against the repo

Every unchecked box was checked against `origin/dev` (`10864178`), the commits on `feat/multi-platform-releases` (a180b6c4 … 21f2e50c) and the artefacts on disk — `git log` per file, `grep` per promised symbol, plus the multi-platform-releases Log. Ticks and `— open:` lines were added in place; no task text was rewritten or removed.

- **Ticked (5)**: 3.1 — `osadapter/posix.py` merged via PR #167 into dev, contract test on three CI legs, VM-proven. 3.2 — `linux.py` (#167) and `darwin.py` (4b7618e5 … c2415865) both on dev, suite green on real macOS. 3.4 — `verify_artifact_family`, the SIGBREAK guard, the ignore lists trimmed, the deb self-update proven on the kiosk. 3.7 — `--preseed` / `pairing.json` / `OWLETTE_ADD` shipped and consumed by both maintainer scripts; the kiosk paired through it, the MBA paired. 4.1 — PRs #211/#212 landed the cfg gating, `tauri.conf.json`, `paths.rs`, `shell_open.rs`, and `rust-build.yml` checks the crate on all three OSes.
- **Left open, code shipped but a named deliverable is missing (9)**: 4.2 (restart via the `ipc/requests` seam), 4.3 (`shell`/`launch` bodies, Linux capture), 4.4 (`capabilities.screenCapture` never reaches the heartbeat), 4.5 (per-OS classifier, case-sensitive `pathKey`), 4.6 (arm64 leg — reserved), 4.7 (pair/restart via the seam), 5.1 (the install, `uninstall.sh`, `pmset`, the management profile), 5.2 (arm64 — reserved; `sleep.target` mask), 5.3 (the tag run), 6.1 (`capabilities.*` keys), 6.4 (preseed, Q24, login items, MDM in the docs). 4.6 and 5.2 are one descoping ruling away from a tick: `linux_arm64` is already reserved by multi-platform-releases decision 1.
- **Left open, not started (2)**: 5.6 (the deploy hook still flattens by basename; needs an explicit request), 0.3 (no Linux capture spike; capture not built, Q5).
- **Left open, owner-side measurement owed (4)**: 0.2 (reboot survival, (c), the 30-day prompt), 0.4 macOS half (reboot, notification), 0.6 (the five-run runner timing; secrets registered and the job graph drawn), 0.7 (prod floor).
- **Could not settle either way (3)**: 0.5 (no spike file ever committed; the matrix it gated shipped), 5.0 and 5.4/5.5 (task text never recovered). Left as they were.
- **Not built anywhere**: Linux screen capture (Q5), Linux arm64, the macOS `com.apple.servicemanagement` profile (Q16).
- **The owner owes**: the pkg install on the MBA with sudo (unblocks 5.1's Done-when, 0.4's macOS half, 3.1/3.4's macOS root checks, 4.2/4.7's macOS runtime proofs); the eight Apple secrets + the `release` environment restricted to `v*` tags on GitHub (unblocks 5.3's tag run and 0.6's runner timing); the Mac reboot check (0.2 grant survival, 0.4 auto-login).

### 2026-09-25 (overnight, inline, owner asleep) — the Mac and the kiosk VM as real test beds

SSH to the MBA (owner enabled Remote Login) and to the Hyper-V kiosk VM (passwordless sudo) turned the posix work from CI-only into measured runtime: spike 0.2 (a)/(b) answered (348 ms to a JPEG on the app's own grant; an ad-hoc grant dies on rebuild; `screencapture` never prompts, the app's own ask does, and an undecided app re-prompts on every retry); the desktop crate 118/118 and the agent suite 1766/225 on real macOS; the macOS app gets native window buttons, a left-click menubar menu, a Dock icon only with its window, and its login-item toggle defers to the installer's LaunchAgent; the job runner (4.3) and the TCC reporter (4.4) written and measured with hand-written jobs and through the daemon's own `posix.capture_screen`/`posix.notify`; an unsigned `.pkg` (5.1) built and inspected; the Linux `.deb`s (5.2) built on the VM, installed over the lab 3.3.5, the kiosk heartbeating to dev as 3.3.15, the polkit rule and the user unit proven, and a reboot proving auto-login; two agent fixes off the Mac run (a Windows-only import on every watcher tick, the runner's host line) and one flaky fixture (the fake job runner re-served requests). Blocked on the owner: pairing the MBA (needs a dashboard session), the Apple team id, a Mac reboot, granting `owlette.app` Screen Recording. Not done: Linux capture (Q5), arm64 Linux, the apt self-update path, `PermissionBanner.tsx`, the `com.apple.servicemanagement` profile, real installs on the Mac (no sudo over ssh).
